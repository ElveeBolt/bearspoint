...
#
# def calc_period(master, service, workday_time_start, workday_time_end) -> list:
#     free_time = []
#     bookings = Booking.objects.filter(master=master, date=workday_time_start.date)
#
#     busy_time = []
#     for booking in bookings:
#         booking_time_start = booking.time
#         booking_time_end = booking.time + timedelta(minutes=booking.service.time)
#
#         busy_time.append(convert_period_to_set(booking_time_start, booking_time_end))
#
#     day_times = convert_period_to_set(workday_time_start, workday_time_end - timedelta(minutes=service.time))
#
#
#
#     for time_start in day_times:
#         possible_time = convert_period_to_set(time_start, time_start + service.time)
#
#         no_intersection = True
#         for one_booking in busy_time:
#             if len(possible_time.intersection(one_booking)) > 1:
